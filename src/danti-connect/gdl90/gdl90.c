...
bool decode_gdl90_traffic_report(gdl_message_t *rawMsg, gdl90_msg_traffic_report_t *decodedMsg) {
    ...
    decodedMsg->trafficAlertStatus = (traffic_alert_status_t) GDL90_DECODE_TRAFFIC_ALERT(rawMsg->data);
    decodedMsg->addressType = (address_type_t) GDL90_DECODE_ADDRESS_TYPE(rawMsg->data);
    ...
    decodedMsg->reportType = (traffic_report_type_t) GDL90_DECODE_REPORT_TYPE(rawMsg->data);
    decodedMsg->ttType = (heading_or_track_type_t ) GDL90_DECODE_HEADING_TRACK_TYPE(rawMsg->data);

    decodedMsg->nic = (nic_t) GDL90_DECODE_NIC(rawMsg->data);
    decodedMsg->nacp = (nacp_t) GDL90_DECODE_NACP(rawMsg->data);
    ...
    decodedMsg->emitterCategory = (emitter_category_t) GDL90_DECODE_EMITTER_CATEGORY(rawMsg->data);

    for (int i=0; i < GDL90_TRAFFICREPORT_MSG_CALLSIGN_SIZE; i++) {
        decodedMsg->callsign[i] = rawMsg->data[GDL90_DECODE_CALLSIGN_START_IDX + i];
    }

    decodedMsg->emergencyCode = (emergency_code_t) GDL90_DECODE_EMERGENCY_CODE(rawMsg->data);
